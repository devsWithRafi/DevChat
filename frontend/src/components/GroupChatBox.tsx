...
const GroupChatBox = () => {
  ...
  return (
    <section
      className={cn(
        'w-full relative h-full flex flex-col justify-between max-[767px]:px-3',
    ...
    >
      ...
      {/* message input */}
      <section className="p-5 pt-0 flex flex-col gap-3 max-[767px]:p-3">
        {isUserInGroup ? (
          <form
            onSubmit={handleSend}
            className={cn(
          ...
          >
            ...
            <div
              className={cn(
                'flex flex-col gap-5 w-full items-center border rounded-full h-12 px-6 justify-center max-[767px]:px-4.5 max-[767px]:h-9',
                imageFile &&
            ...
            <Button
              type="submit"
              disabled={sendButtonDesabled || isSending}
              className={cn(
          ...
          <Button
            ...
            className="w-full rounded-full p-2.5 cursor-pointer text-sm flex items-center justify-center"
          >
            {!loading ? 'Join Group' : <LuLoader className="size-5 animate-spin"/>}
          </Button>
        )}
      </section>
    </section>
