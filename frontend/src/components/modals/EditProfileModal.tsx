...
const EditProfileModal = ({ setOnEditClick }: EditProfileModalProps) => {
  ...
  const avaterImagePreview = avaterFile
      ...
      <section className="w-200 bg-white/15 border border-white/15 p-7 rounded-xl flex flex-col shadow-2xl max-[825px]:w-[95vw]">
        ...
        <div className="flex gap-7 w-full mt-5 items-start max-[825px]:flex-col max-[825px]:items-center">
          <div className="min-w-55 max-w-55 max-[500px]:w-full max-[500px]:min-w-full aspect-square overflow-hidden relative cursor-pointer group">
            ...
            <label
              htmlFor="avaterFile"
              className="absolute duration-300 bg-black/20 flex items-center justify-center text-[25px] cursor-pointer text-white top-0 left-0 right-0 bottom-0 h-full w-full"
            >
              <MdOutlineCameraAlt />
            </label>
            <input
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setInputValues((prev) => ({ ...prev, bio: e.target.value }))
              }
            ...
            <button
              type="submit"
              className={cn(
                'bg-white w-full text-black p-2 rounded-full mt-5 font-medium text-md cursor-pointer hover:bg-gray-100 flex items-center justify-center gap-2',
              )}
            >
              {loading ? <Spinner className="border-2 h-5 w-5" /> : 'Save'}
            </button>
          </form>
        </div>
      </section>
